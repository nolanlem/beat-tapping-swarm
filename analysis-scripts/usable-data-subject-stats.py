...
batch_folder = 'usable-batch-11-8'###### USABLE batch where you've put the downloaded pavlovia experiment dirs
csvfiles = []
...
for file in glob.glob('./psychopy/swarm-tapping-study/mturk-csv/' + batch_folder + "/*.csv"):
    print(file)
...
#%% NB: don't have to run.... separate subjects from usable batches tostanford-friends vs. mturk 

stanford_usable_dir = './psychopy/swarm-tapping-study/mturk-csv/stanford-batch/'
mturk_usable_dir = './psychopy/swarm-tapping-study/mturk-csv/mturk-batch/A1/'

if os.path.exists(stanford_usable_dir) == False:
    os.mkdir(stanford_dir)
if os.path.exists(mturk_usable_dir) == False:
    os.mkdir(mturk_usable_dir)

## add the files to the respective dirs
for csvfile in email_subjects:
    new_loc = os.path.join(stanford_usable_dir, os.path.basename(csvfile))
    copyfile(csvfile, new_loc)
    
for csvfile in mturk_subjects:
    new_loc = os.path.join(mturk_usable_dir, os.path.basename(csvfile))
    copyfile(csvfile, new_loc)
...
csv_filename = './psychopy/swarm-tapping-study/mturk-csv/' + batch_folder + '/stats/' + batch_folder + '-' + time_tag + '-participant-STATS.csv'
...
with open(csv_filename, 'w', newline= '') as file: 
    ...
    for csv_file in glob.glob(os.path.join('./psychopy/swarm-tapping-study/mturk-csv/', batch_folder, "*.csv")):
...
cnt = 0
for fi in glob.glob('./psychopy/swarm-tapping-study/mturk-csv/' + batch_folder + '/*.csv'):
    csvfile = pd.read_csv(fi, keep_default_na=False)
    
    experiment_version = os.path.basename(fi).split('.')[0].split('_')[1].split('-')[1]
    participant_id = csvfile['Participant Initials'][0]
    #print(experiment_version)
    email = ''
    mturk_id = ''
    ...
    ### if no email and no mturk given, assume it's a stanford/friend person 
    if mturk_id == '' and email == '':
        stanford_participants[experiment_version].append('NA')
        
    print(csvfile['Participant Initials'][0], mturk_id, email, experiment_version,os.path.basename(fi))
    cnt +=1 
