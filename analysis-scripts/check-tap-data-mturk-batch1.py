...
for csv_file in csvfiles:
    ...
    if (mturk_id != 'none') and (mturk_id != ''):
        subjectplotdir = './psychopy/swarm-tapping-study/analysis-scripts/plots/' + batch_num + '/subjects/' + mturk_id        
        ...
        if (email != 'none') and (email != ''):
            print(email)
            subjectplotdir = './psychopy/swarm-tapping-study/analysis-scripts/plots/' + batch_num + '/subjects/' + email
            if os.path.exists(subjectplotdir) == False:
                os.mkdir(subjectplotdir)
            ...
            if (inits != 'none') and (inits != ''):
                subjectplotdir = './psychopy/swarm-tapping-study/analysis-scripts/plots/' + batch_num + '/subjects/' + inits
                if os.path.exists(subjectplotdir) == False:
                    os.mkdir(subjectplotdir)
                    subject[csv_file_basename] = inits                    
            else:
                print(csv_file)
                subjectplotdir = './psychopy/swarm-tapping-study/analysis-scripts/plots/' + batch_num + '/subjects/' + csv_file
                if os.path.exists(subjectplotdir) == False:
                    os.mkdir(subjectplotdir)
...
#%%
# ['TF',
#  'QR', # just did practice trials no spacebar presses 
#  'Randy',
#  'GAIL TELFAIR',
#  'TF',
#  'M',
...
for person in subject:
    ...
    plt.savefig(os.path.join(subjectplotdir, subject[person], person + ' taps.png'), dpi=120)
    plt.close()
    print('\n\n')
