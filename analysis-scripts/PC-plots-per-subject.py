...
# R_subj_cols = ['R ' + elem for elem in subject]
# psi_subj_cols = ['PSI ' + elem for elem in subject]
# header = R_subj_cols + psi_subj_cols

# header[:0] = ['coupling condition', 'beat section', 'timbre version']
...
## remove bad subjects? this was taken from check_badsubjects.py script... 
...
for badsubject in badsubjects:
    if badsubject in subject:
        subject.remove(badsubject.split('.')[0])

for person in subject:
    R_csv = open('./psychopy/swarm-tapping-study/analysis-scripts/plots/beat-segment-analysis/' + str(len(beatsegments)) + '-beat-segments/PCs/subject_csvs/' + person + '.csv', 'w')
    R_writer = csv.writer(R_csv)
    R_writer.writerow(['coupling condition', 'beat section', 'timbre version', 'R', 'psi'])
    
    subject_ph[person] = {}
    for version, v in zip(all_timbre_conds, ['t','n']):
        subject_ph[person][v] = {}
        for sync_cond, sync_str in zip(version, sync_conds):
            subject_ph[person][v][sync_str] = []
            ...
            for n, sync_cond_version in enumerate(sync_cond):
                ...
                try:
                    ...
                    subject_ph[person][v][sync_str].append(subject_binned_taps_per_stim[person])
      
                except:
                    pass
            
            df_subject = pd.DataFrame(subject_ph[person][v][sync_str])     # (470,19) = (5*tempo*2versions * 47 subjects) for coupling cond and timbre version                        
            ...
            for m, beatwindow in enumerate(beatsegments):
                ...
                psi_subject =  (psi_subject + 2*np.pi)%(2*np.pi)
                R_writer.writerow([sync_str, str(m), v, R_subject, psi_subject])
            
    R_csv.close()
#%%#########NB: now run plot_r_psi.py to plot the csv files that were generated #####################
######################################################################################################
...
for version,v in zip(all_timbre_conds, ['t','n']):
    ...
    for sync_cond, sync_str in zip(version, sync_conds):
        ...
        for m, beatwindow in enumerate(beatsegments):
            ...
            #subject_beat_col_sd = np.nanmean(subject_beat_column_mx)
            ...
            ax_model[sc + vc,m].arrow(0, 0.0, psi_model_centered, R_model, color='black', linewidth=1)
            ...
            ax_combined[sc + vc,m].arrow(0, 0.0, psi_model_centered, R_model, color='black', linewidth=0.7, zorder=1)
