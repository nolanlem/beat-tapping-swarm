...
for stimuli_dir in stimuli_dirs:
    ...
    for fi in glob.glob(beatbins_dir + '/*.txt'):
        ...
        sndbeatbins[fi_basename] = thebeatbins
    
    centerbpms_dir = os.path.join(datadir, stimuli_dir, 'phases', 'center-bpm')    
    for fi in glob.glob(centerbpms_dir + '/*.txt'):       
        fi_basename = str.split(os.path.basename(fi), '.')[0] # --> weak_79_1        
        thecenterbpm = np.loadtxt(fi)        
        centerbpms[fi_basename] = float(thecenterbpm)
        centerperiods[fi_basename] = 60./float(thecenterbpm)
# old code
...
for csv_file, person in zip(csvfiles, subject):
    ...
    subject_resps[person]['no'] = {}
    subject_resps[person]['timbre'] = {}    
    ...
    df_block_1 = df_block.get([csv_participant, csv_type, csv_coupling_cond, csv_tempo, csv_version, block1taps])[4:44]
    df_block_2 = df_block.get([csv_participant, csv_type, csv_coupling_cond, csv_tempo, csv_version, block2taps])[44:]
    
    
    ######## version A1
    if experiment_version == 'A1':
        for index, row in df_block_1.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            #print(sync_cond_version)
            subject_resps[person]['no'][sync_cond_version] = []
        for index, row in df_block_2.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            #print(sync_cond_version)
            subject_resps[person]['timbre'][sync_cond_version] = []
    ######## version A2
    if experiment_version == 'A2':
        for index, row in df_block_1.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            subject_resps[person]['no'][sync_cond_version] = []
        for index, row in df_block_2.iterrows():
...
#########
    ...
    if experiment_version == 'A1':
        for index, row in df_block_1.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            subject_resps[person]['no'][sync_cond_version] = removeStrFormatting(row[block1taps])
        for index, row in df_block_2.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            subject_resps[person]['timbre'][sync_cond_version] = removeStrFormatting(row[block2taps])
    ######## version A2
    if experiment_version == 'A2':
        for index, row in df_block_1.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
    ...
    if experiment_version == 'B1':
        for index, row in df_block_1.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            subject_resps[person]['timbre'][sync_cond_version] = removeStrFormatting(row[block1taps])
        for index, row in df_block_2.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
            subject_resps[person]['no'][sync_cond_version] = removeStrFormatting(row[block2taps])             
    ######## version B2            
    if experiment_version == 'B2':
        for index, row in df_block_1.iterrows():
            sync_cond_version = str(row[csv_coupling_cond] + "_" + str(row[csv_tempo]) + "_" + str(row[csv_version]))
...
tap_snds_no = list(subject_resps[person]['no'].keys())

        

for n, taps in enumerate(tap_snds_no):
    plt.figure()
    fig, ax = plt.subplots(len(tap_snds_no), 1, sharex=True, sharey=True, figsize=(20,20))
    ...
    for sndfile in taps:
        ...
        try:
            ...
            subjecttaps = librosa.time_to_samples(subject_resps[person]['no'][sync_cond_version]) #need to redefine [(no,timbre)] to look at timbre par exemple
            
            ax[i].plot(y, linewidth=0.5) # plot sound waveform
            ax[i].vlines(subjecttaps, -0.5, 0.5, color='red') # plot subject taps        
            ax[i].vlines(sndbeatbins[sync_cond_version], -1, 1, color='green') # plot beat bins 
            
            ax[i].set_yticks([]) # turn off y ticks
            i += 1
        except KeyError:
            print('could not find %r'%(sndfile))

    fig.suptitle(person + ' Block ' + str(n+1) + ' taps ')
    plt.savefig('./plots/subject-taps/' + person + '-t-block_ ' + str(n+1) + '-taps.png', dpi=120)
