...
for file in glob.glob('./psychopy/swarm-tapping-study/mturk-csv/usable-batch/*.csv'):
    csvfiles.append(file)
...
#     subjectplotdir = './psychopy/swarm-tapping-study/analysis-scripts/plots/' + batch_folder + '/subjects/' + participant
...
#%% ########## GET ALL STIM NAMES from allstims dir --> allstims list
allstims = []
...
for cnt, person in enumerate(subject):
    ...
    plt.figure()
    fig, ax = plt.subplots(80, 1, sharex=True, sharey=True, figsize=(20,40))
    n = 0
...
notimbre_conds = [n_none, n_none, n_medium, n_strong]
...
avg_binned_taps = {}
subject_std_slices = {}
subject_all_iti_taps = {}
subject_sync_cond = {} # just to keep reference to sndfile 
iti = {}
std = {}
    ...
    for sync_cond, sync_str in zip(sndfiles_batch, sndfile_conds):
        ...
        for sync_cond_version in sync_cond:
            ...
            try:
                ...
                subject_all_iti_taps[person][sync_str].append(normalized_tap_iti)
                subject_sync_cond[person][sync_str].append(sync_cond_version)
        ...
        df_subject_iti_per_cond = pd.DataFrame(subject_all_iti_taps[person][sync_str])
        subject_mean_iti_per_tap = df_subject_iti_per_cond.mean(axis=0)
        subject_std_iti_per_tap = df_subject_iti_per_cond.std(axis=0)
        #plotSubjectITIperTap(subject_mean_iti_per_tap, subject_std_iti_per_tap, sync_str, person)
...
##############################################################   
########### ITI PLOTS PER BEAT (NO BEAT SECTIONING) ###################
##############################################################

iti_conds = {'none': [], 'weak': [], 'medium': [], 'strong': []}

for person in subject:
    for sync_str in sndfile_conds:
        iti_conds[sync_str].extend(subject_all_iti_taps[person][sync_str])
iti_df = {}
for sync_str in sndfile_conds:
    iti_df[sync_str] = pd.DataFrame(iti_conds[sync_str])
...
iti_sx = {}
for sync_str in sndfile_conds:
    iti_mx[sync_str] = iti_df[sync_str].mean(axis=0)
    iti_sx[sync_str] = iti_df[sync_str].std(axis=0)
...
for sync_str in sndfile_conds:
    ax.plot(iti_mx[sync_str], linewidth=0.8)
    
    xrange = np.linspace(0, len(iti_mx[sync_str])-1, len(iti_mx[sync_str]))
    ax.errorbar(xrange, iti_mx[sync_str], yerr=iti_sx[sync_str], label=sync_str, marker='.', capsize=3)

ax.set_xticklabels([])
ax.set_ylabel('mean ITI')
ax.set_xlabel('beat')
#ax.set_ylim([0.3, 2.0])
ax.set_title('Mean ITI Per Beat')
plt.legend(title='coupling conditions', bbox_to_anchor=(1., 1.05))
plt.savefig('./psychopy/swarm-tapping-study/analysis-scripts/plots/ITIs/all-beats/' + suptitle_str + ' all-beats.png', bbox_inches="tight", dpi=160)
plt.tight_layout()
##############################################################   
########### ITI PLOTS WITH BEAT SECTIONING ###################
##############################################################
# 4, 4-beat segments 
beatsegments = [(0,4),(4,8),(8,12),(12,16), (16, 20)]

iti_segment_mx, iti_segment_mx_error = {}, {}
iti_segment_sx, iti_segment_sx_error = {}, {}

iti_mx_segments_all, iti_mx_error_segments_all = {}, {}
iti_sx_segments_all, iti_sx_error_segments_all = {}, {}

for sync_str in sndfile_conds:
    iti_mx_segments_all[sync_str] = []
    iti_mx_error_segments_all[sync_str] = []
    iti_sx_segments_all[sync_str] = []
    iti_sx_error_segments_all[sync_str] = []
    
    for beatseg in beatsegments:
        beat_column = iti_df[sync_str].iloc[:, beatseg[0]:beatseg[1]].values
        iti_segment_mx[sync_str] = np.nanmean(beat_column)
        
        iti_mx_segments_all[sync_str].append(iti_segment_mx[sync_str])
        
        #iti_segment_mx_error[sync_str] = beat_column.mean(axis=1).std()
        iti_segment_mx_error[sync_str] = np.nanstd(np.nanmean(beat_column, axis=1))
        
        iti_mx_error_segments_all[sync_str].append(iti_segment_mx_error[sync_str])
        
        iti_segment_sx[sync_str] = np.nanstd(beat_column)
        iti_sx_segments_all[sync_str].append(iti_segment_sx[sync_str])
        iti_segment_sx_error[sync_str] = np.nanstd(np.nanstd(beat_column, axis=1))
        iti_sx_error_segments_all[sync_str].append(iti_segment_sx_error[sync_str])
...
xlabels_pos = np.arange(0,5)
xlabels = [str(elem) for elem in beatsegments]

for sync_str in sndfile_conds:
    ax[0].plot(iti_mx_segments_all[sync_str], linewidth=0.8)
    
    xrange = np.linspace(0, len(iti_mx_segments_all[sync_str])-1, len(iti_mx_segments_all[sync_str]))
    ax[0].errorbar(xrange, iti_mx_segments_all[sync_str], yerr=iti_mx_error_segments_all[sync_str], label=sync_str, marker='.', capsize=3)    

    ax[1].plot(iti_sx_segments_all[sync_str])
    xrange = np.linspace(0, len(iti_sx_segments_all[sync_str])-1, len(iti_sx_segments_all[sync_str]))
    ax[1].errorbar(xrange, iti_sx_segments_all[sync_str], yerr=iti_sx_error_segments_all[sync_str], label=sync_str, marker='.', capsize=3)    
      
    # titles and stuff 
    ax[0].set_title(suptitle_str + ' Mean ITI')
    ax[1].set_title(suptitle_str + ' STD ITI')

    ax[0].set_xticks(xlabels_pos)
    ax[1].set_xticklabels(xlabels)
    ax[1].set_xlabel('beat segment')

plt.legend(title='coupling conditions', bbox_to_anchor=(1., 1.05))
plt.savefig('./psychopy/swarm-tapping-study/analysis-scripts/plots/ITIs/5-beat-sections/' + suptitle_str + ' 5-beat-sections.png', dpi=160, bbox_inches="tight")
    
#%% make csv file for beat segments case 
...
iti_mx_vect, iti_mx_error_vect = {}, {}
iti_sx_vect, iti_sx_error_vect = {}, {}

beat_strs = ['0','1','2','3','4']
import csv
with open('./psychopy/swarm-tapping-study/csv/csv-output-10-7.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["subject", "study", "version", "condition", "section", "mx", "sx"])
    
    for sync_str in sndfile_conds:
        
        iti_mx[sync_str], iti_mx_error[sync_str] = {}, {}
        iti_sx[sync_str], iti_sx_error[sync_str] = {}, {}
        
        # this are the vectors for plotting 
        iti_mx_vect[sync_str], iti_mx_error_vect[sync_str] = [], []
        iti_sx_vect[sync_str], iti_sx_error_vect[sync_str] = [], []
        
        
        # loop over beatsegs just to instantiate dictionaries
        for beatseg in beatsegments:
            iti_mx[sync_str][str(beatseg)] = []
            iti_mx_error[sync_str][str(beatseg)] = []
            iti_sx[sync_str][str(beatseg)] = [] 
            iti_sx_error[sync_str][str(beatseg)] = []
    
        
        for person in subject:
            study_number = person.split('_')[1].split('-')[1]
            
            taps_df = pd.DataFrame(subject_all_iti_taps[person][sync_str])        
            
            for beatseg, beat_str in zip(beatsegments, beat_strs):
                tap_column = taps_df.iloc[:,beatseg[0]:beatseg[1]].values
                
                mx = np.nanmean(tap_column)            
                iti_mx[sync_str][str(beatseg)].append(mx)
                
                sx = np.nanstd(tap_column)
                iti_sx[sync_str][str(beatseg)].append(sx)
                
                writer.writerow([person, study_number, version, sync_str, beat_str, mx, sx])
        
        for beatseg in beatsegments:
            
            iti_mx_error[sync_str][str(beatseg)] = np.nanstd(iti_mx[sync_str][str(beatseg)])
            iti_sx_error[sync_str][str(beatseg)] = np.nanstd(iti_sx[sync_str][str(beatseg)])
            
            iti_mx_vect[sync_str].append(np.nanmean(iti_mx[sync_str][str(beatseg)]))
            iti_sx_vect[sync_str].append(np.nanmean(iti_sx[sync_str][str(beatseg)]))
     
            iti_mx_error_vect[sync_str].append(iti_mx_error[sync_str][str(beatseg)])
            iti_sx_error_vect[sync_str].append(iti_sx_error[sync_str][str(beatseg)])        
...
for sync_str in sndfile_conds:
    xrange = np.arange(0, 5)

    ax[0].plot(iti_mx_vect[sync_str])
    ax[0].errorbar(xrange, iti_mx_vect[sync_str], yerr=iti_mx_error_vect[sync_str], label=sync_str, marker='.', capsize=3)
    
    ax[1].plot(iti_sx_vect[sync_str])
    ax[1].errorbar(xrange, iti_sx_vect[sync_str], yerr=iti_sx_error_vect[sync_str], label=sync_str, marker='.', capsize=3)
    ...
    ax[1].set_xticklabels(xlabels)
    ax[1].set_xlabel('beat segment') 
    ax[0].set_ylabel('normalized ITI')
    ax[1].set_ylabel('normalized ITI')
        
plt.legend(title='coupling conditions', bbox_to_anchor=(1., 1.05))
plt.savefig('./psychopy/swarm-tapping-study/analysis-scripts/plots/ITIs/5-beat-sections/' + suptitle_str + 'rev 5-beat-sections.png', dpi=160, bbox_inches="tight")
...
#%%    
            ...
            # subject_iti_error = np.std(tap_colum)
            # iti_bsegs.append(subject_iti)
...
the_time = '_'.join([str(datetime.datetime.now().hour) + "-" + str(datetime.datetime.now().minute), str(datetime.datetime.now().month), str(datetime.datetime.now().day), str(datetime.datetime.now().year)])


with open('./psychopy/swarm-tapping-study/csv/' + suptitle_str + '-csv-output-' + the_time + '.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    ...
    for person in subject:
        ...
        for sync_cond, sync_str in zip(sndfiles_batch, sndfile_conds):
            ...
            for sync_cond_version in sync_cond:
                #print(person, sync_cond_version)
                try:
...
#%% NB: DON'T HAVE TO RUN..... plot histograms of tap densities over waveform stimuli
...
plt.savefig('./psychopy/swarm-tapping-study/analysis-scripts/plots/ITIs/' + suptitle_str + ' ' + the_time + ' ITI.png', dpi=160, bbox_inches='tight')
