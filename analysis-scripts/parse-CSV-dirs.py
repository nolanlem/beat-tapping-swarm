...
### USAGE: 
    1) This scripts parses through the pavlovia repositories from each experiment version A1, A2, B1, B2
    2) It takes note of the total subjects handedness, gender, musical training, age, headphones 
    3) Then it filters out the participants based off of the number of taps for the 'strong' stimuli 
        and copies the 'good' participants to a new dir, 'usable-[batch date]' in './mturk-csv/' 
    ... after this script, we can run ITI.py on the usable-batch data           
@author: nolanlem
"""
...
### 1) reads from the csv file generated with the participant info in order to filter out badsubjects
### 2) creates new directory 'usable-[batch_folder]' with tap responses that meet the criteria:
        ### CRITERIA: have to have 90-110% of the number of ideal taps for the strong stimuli (NB: this is  NOT based off of ITI, that's later )
...
#%%%% NOW NEED TO RUN ITI script to get subject I
