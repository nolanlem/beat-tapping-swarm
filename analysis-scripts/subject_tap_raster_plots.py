...
for version, v in zip(all_timbre_conds, t_strs):
    for sync_cond, sync_str in zip(version, sync_conds):
        for sync_cond_version in sync_cond: 
            raster_mat[sync_cond_version] = []
            for person in subject:
                try:
                    tap_resps = subject_resps[person][sync_cond_version]
                    raster_mat[sync_cond_version].append(tap_resps)
                except:
                    pass
#%% MAKE PLOT DENSITIES PER STIMULUS OF ALL USABLE SUBJECTS TAPS 
#num2plot=5
#%%
spike_blocks = np.linspace(0, spikes.shape[1], int(spikes.shape[1]/2205))
...
def running_mean(x, N):
    cumsum = np.cumsum(np.insert(x, 0, 0)) # add zero to beginning since it cuts off one samp
...
fig, ax = plt.subplots(nrows=20, ncols=1, figsize=(20,15), sharey=True)

density_dir = './psychopy/swarm-tapping-study/analysis-scripts/plots/tap-density-plots/'

#all_none_timbre = [[all_timbre_conds[0][0]]]
all_strong_timbre = [[all_timbre_conds[0][3]]]

from scipy.ndimage import gaussian_filter1d

for version, v in zip(all_timbre_conds, t_strs):
    for sync_cond, sync_str in zip(version, sync_conds):
        #print('working on ', v, sync_cond)
        
        fig, ax = plt.subplots(nrows=20, ncols=1, figsize=(20,25))
        
        for i,sync_cond_version in enumerate(sync_cond):
            print('working on ', sync_cond_version)
            y, sr_ = librosa.load('./psychopy/swarm-tapping-study/allstims/' + sync_cond_version + '.wav')
                      
            tap_mat = [librosa.time_to_samples(arr) for arr in raster_mat[sync_cond_version]]
            tap_mat = [[tap for tap in taps if tap <= len(y)] for taps in tap_mat]  # remove taps greater than len of audio stim          
            tap_mat = pd.DataFrame(tap_mat).values
            
            x_v = [[1 + elem/5.] for elem in range(tap_mat.shape[0])] # vertical plot displacement per tap 
            x_r = np.ones((tap_mat.shape[0], tap_mat.shape[1]))*np.array(x_v)
            ax[i].plot(tap_mat, x_r, marker='s', markersize = 1, linestyle='none')
            ax[i].plot(y, linewidth=0.5)
            # plot beat bins
            ax[i].vlines(sndbeatbins[sync_cond_version], -2, 5, color='red', linewidth=0.88)
            ax[i].set_title(sync_cond_version, fontsize=8)
            ax[i].set_xticks([])
            ...
            spikes_mx = np.nanmean(spikes[:, :len(y)-1], axis=0)
            mx = running_mean(spikes_mx, 1) # take moving average over around 0.3(6000 samps) sec intervals 
            mx = mx/max(mx)     # normalize it for plotting
            mx_pad = np.pad(mx, (0,len(y)-len(mx)), 'constant', constant_values=(0))
            #ax[i].plot(mx_pad)
            
            # TAKE PER STIMULUS TIME HISTORGRAM 
            mx = PSTH(spikes, 1102, len(y)) # per stimulus time histogram, 2205 = 0.1 seconds 
            mx = mx/max(mx) # normalize 
            mx_ = mx - 1
            mx_ = mx_*2
            ax[i].plot(mx_, linewidth=0.5)
            # kernel density estimation with gaussian kernel 
            gmx = gaussian_filter1d(mx, 1000)
            gmx = gmx/max(gmx)
            gmx *= 2
            gmx -= 2
            ax[i].plot(gmx, linewidth=0.8, color='red')
            # plot 'center of beat' with black vertical line 
            bbmid = np.diff(sndbeatbins[sync_cond_version])
            for bb, bbm in zip(sndbeatbins[sync_cond_version], bbmid):
                ax[i].vlines(bb + int(bbm/2), 0.2, -2, color='black', linewidth=0.6)
        
        finame = v + ' ' + sync_str + ' tap density.png'
        fig.suptitle(v + ' ' + sync_str + ' Tap Density')
        fig.tight_layout()
        #fig.savefig(os.path.join(density_dir, finame), dpi=150)
#%%
plt.plot(y)
plt.plot(gmx)
...
tap_mat = pd.DataFrame(raster_mat[sync_cond_version]).values
tap_mat[np.isnan(tap_mat)] = len(y)/float(sr_) # remove nans, replace with last sample num

y, sr_ = librosa.load('./psychopy/swarm-tapping-study/allstims/' + sync_cond_version + '.wav')
tap_mat = [librosa.time_to_samples(taps) for taps in tap_mat]
tap_mat = np.array(tap_mat)
spikes = np.zeros((tap_mat.shape[0], 1+int(np.amax(tap_mat))))
for i,taps in enumerate(tap_mat):
    np.put(spikes[i],taps,1)

N = 10
spike_mx_mean = []
for i in range(0, len(y)-1, 1):
    mx = np.mean(spikes[:,i:(i+N)])
    spike_mx_mean.append(mx)  
plt.plot(spike_mx_mean)
...
# for version, v in zip(all_timbre_conds[1], t_strs):
#     for sync_cond, sync_str in zip(version, sync_conds):
#         for i,sync_cond_version in enumerate(sync_cond[:num2plot]):
#             tap_mat = pd.DataFrame(raster_mat[sync_cond_version]).values
#             ax[i].plot(tap_mat, marker='.', linestyle='none')            
